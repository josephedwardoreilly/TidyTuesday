...
bins <- c(0, 0.001, 0.01, 0.1,
          1, 10, 100)
z[, bin := as.character(.bincode(
  forest_area,
  bins,
  include.lowest = TRUE))] # include forest_area == 0 in lowest bin

z[code == 'ATA', bin := '1'] # Antartica has no forest
...
# Legend 
legend <- data.table(level = c(NA, sort(z[,unique(bin)])),
                 xmin = c(0:6),
                 xmax = c(1:7), 
